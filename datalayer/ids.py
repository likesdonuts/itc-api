...
module does nothing but get a copy of it and keep the copies tidy:
    ...
    data/ids/investigations-2026-09-22.json.gz

One snapshot per calendar day, gzipped (37 MB becomes about 3 MB), never
rewritten once taken. Everything downstream reads a snapshot rather than the
network, so parsing, schema changes and re-rendering all work offline and
always against a file you can go back to.
"""
...
def snapshots(ids_dir: Path = IDS_DIR) -> list[Snapshot]:
    """Every stored snapshot, oldest first (the names sort chronologically)."""
    directory = Path(ids_dir)
    if not directory.is_dir():
        return []
    found = sorted(directory.glob(f"{PREFIX}*{SUFFIX}"))
    return [Snapshot(path=path, day=snapshot_day(path)) for path in found]
...
def snapshot_for(day: str, ids_dir: Path = IDS_DIR) -> Snapshot | None:
    path = Path(ids_dir) / f"{PREFIX}{day}{SUFFIX}"
    return Snapshot(path=path, day=day) if path.exists() else None
...
def store(raw: bytes, *, day: str | None = None, ids_dir: Path = IDS_DIR) -> Snapshot:
    """Gzip the downloaded bytes into today's snapshot, after checking they
    parse -- a snapshot on disk should always be usable.
    """
    try:
    ...
    path = directory / f"{PREFIX}{day}{SUFFIX}"
    tmp = path.with_name(f".{path.name}.tmp")
    with gzip.open(tmp, "wt", encoding="utf-8") as handle:
        json.dump(payload, handle)
    os.replace(tmp, path)
    return Snapshot(path=path, day=day)
...
def sync(
    *,
    ...
    """
    day = day or date_cls.today().isoformat()
    existing = snapshot_for(day, ids_dir)
    ...
    if existing and not force:
        ...
        snapshot = store(download(url), day=day, ids_dir=ids_dir)
        size_mb = snapshot.path.stat().st_size / 1_000_000
        log(f"  stored {snapshot.path.name} ({size_mb:.1f} MB gzipped).")
        downloaded = True
