...
def render_index(investigations: list[dict[str, Any]]) -> str:
    ...
    if not rows:
        body_rows = ""
        empty_state = '<div class="empty-state">No investigations tracked yet. Run <code>python refresh.py</code>.</div>'
    else:
        empty_state = ""
        row_html = []
        for inv in rows:
...
<p class="footer-note">Generated by refresh.py from EDIS &middot; {len(rows)} investigation(s) tracked.</p>
"""
